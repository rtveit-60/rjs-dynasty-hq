...
- **Highschool Recruiting.** Every prospect in the class with stars, true overall, height and weight, gem/bust, dev trait, pipeline, position and national rank, offers and commit tracking. A committed recruit's row carries a ⇄ **Swap Commitment** button that moves the commitment to any school, your program first in the list, recorded the way the game records its own commitments: the new school leads the recruit's list and holds a scholarship on them. A Scheme Fit dot shows how the recruit's archetype sits in your actual scheme, filled when your scheme starts that archetype, from the game's own per-scheme preferences. The Edge arrow scores your program against the strongest school actually pursuing them: green when you hold a real advantage, red when you're behind. Clicking any recruit expands their At a Glance card with two rows of ratings, Physical (speed, acceleration, agility, change of direction, strength, jumping, stamina, toughness, injury) and Positional (the skills their position lives on), plus their mental and physical abilities, and their motivations and ideal pitch.
...
The board is split into tabs by role, head coaches, offensive coordinators and defensive coordinators, each sortable by any column. Once the season ends and the carousel starts turning, an **Open Jobs** tab appears with every vacancy, why it opened, and who filled it for how much.
...
**Coach prestige regression.** The game pays every win with prestige and takes back a single point per loss, and nothing at all for a firing or a missed expectation, so by the eighth season a third of the country's head coaches sit at A+ with nowhere to fall. The **Prestige Ledger** on this tab adds the other side of the ledger. Pick a tier in Setup (Off, Lenient, Balanced, Demanding, Ruthless) and on each sync that brings a new week the app deducts prestige for what happened: every loss weighed by how badly it reads (a ranked unbeaten losing to a 1–7 team costs several times what two heavyweights trading a game does, then postseason, rivalry, home field and margin adjust it), losses that compound while a losing streak runs, seasons that close on the hot seat, and firings. Coordinators pay a share of their head coach's charge; human coaches are treated like everyone else. Each charge is the larger of a flat amount and a share of the coach's own score, so a long winning run still buys a cushion and nothing is ever clamped. The ledger lists every deduction with its reason and before/after score, and the carousel's Prestige column carries the season's total. Off is the default and writes nothing; any other tier writes only the `_RJ` copy, so play the `_RJ` file to keep the game and the app on one save.
...
- **Knows whose dynasty it is.** Loading a save selects the program you're coaching automatically. A save with several user-controlled teams asks which one is yours. When your coach takes a new job, the dashboard follows them to the new program.
...
The editors follow the same rule by writing somewhere else entirely. Using **Edit** creates a separate copy of your dynasty named `<save>_RJ` next to the original and puts the changes there. Your original file keeps its exact bytes. Re-editing an edited copy updates that copy in place, after a timestamped backup is stored in the app's data folder. Load the `_RJ` save in the game to play with your changes. Every save-writing feature (the player and coach editors, depth chart drag-and-drop, board edits, weekly plans, Instant Commit, Mass Commit, Swap Commitment, program grades, pipelines, facilities, resources, Dynasty Settings, Fire Coach, Create Recruit, Manual Transfers) goes through this one path. So does the one automatic writer, coach prestige regression: it is off until you pick a tier in Setup, and when on it writes the `_RJ` copy after a sync brings a new week, with the same backup and read-back verification as a hand edit.
