...
The editors follow the same rule by writing somewhere else entirely. Using **Edit** creates a separate copy of your dynasty named `<save>_RJ` next to the original and puts the changes there. Your original file keeps its exact bytes. Re-editing an edited copy updates that copy in place, after a timestamped backup is stored in the app's data folder. Load the `_RJ` save in the game to play with your changes. Every save-writing feature (the player and coach editors, depth chart drag-and-drop, board edits, weekly plans, Instant Commit, Mass Commit, Swap Commitment, program grades, facilities, resources, Dynasty Settings, Fire Coach, Create Recruit, Manual Transfers) goes through this one path. So does the one automatic writer, coach prestige regression: it is off until you pick a tier in Setup, and when on it writes the `_RJ` copy after a sync brings a new week, with the same backup and read-back verification as a hand edit.
