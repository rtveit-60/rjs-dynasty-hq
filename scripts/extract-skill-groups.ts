...
// (The buckets' SPCost / MaxPotentialCost splines are deliberately not
// emitted: the game's spend routine is compiled, not data, so their indexing
// is unverified — see RESEARCH "Player skill groups & caps".)
interface BucketDef {
  name: string;
  primary: SkillDef[];
  secondary: SkillDef[];
...
lines.push(' * SkillGroupCap slots names, the skills (save rating fields) it levels and its');
lines.push(' * UI color.');
lines.push(' *');
...
for (const k of keys) {
  ...
  for (const b of groups[k]) {
    lines.push(
      `    { name: ${JSON.stringify(b.name)}, primary: ${refs(b.primary)}, secondary: ${refs(b.secondary)}, tertiary: ${refs(b.tertiary)}, rgb: [${b.rgb.join(', ')}] },`
    );
  }
  lines.push('  ],');
}
