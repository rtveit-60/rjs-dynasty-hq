...
- Why name search cannot reach it: the install is 271 `.cas` archives / ~47 GB, all Oodle-compressed, and franchise data almost certainly sits in **unnamed chunks** (`Win32/globals` alone declares 3,117 chunks against 31 named bundles). Chunks are addressed by GUID, so finding the goal table means bulk-decompressing and fingerprinting chunks — the same wall `Team.City` hit, and a project on the scale of the playbook extraction. The `coach-goals.ts` id→text map exists precisely so this is not required.
