...
- Frostbite search came up empty for a goal-definition bundle: nothing under `Win32/globals` (31 bundles, all gameplay/player banks) or by name across all 44 superbundles — only icons (`dynas_programpoints_contractgoals_…`). Resolving these properly needs FTC container parsing, which does not exist yet.
